...
def get_reporter_role(playfab_id):
    ...
    for id_list, role in checks:
        if playfab_id in id_list:
            return role
    return None
...
def ban_user_playfab(playfab_id, duration_hours, reason):
    ...
    try:
        ...
        resp.raise_for_status()
        logging.info(f"banned {playfab_id} for {duration_hours}h: {reason}")
        return True
    except Exception as e:
...
def handle_report():
    ...
    # non-reportable check
    if target_id in NON_REPORTABLE_IDS:
        reporter_name = get_player_display_name(reporter_id)
...
def list_insta_bans():
    ...
    return jsonify({"insta_bans": list(members)}), 200
